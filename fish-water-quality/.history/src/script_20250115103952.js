(function () {
    ...
    function updateAdditionalInfo(temp, ph, length) {
        ...
        document.getElementById("timestamp-value").textContent = new Date().toLocaleTimeString();
    }
    ...
    function generateFishTank() {
        ...
        fishContainer.innerHTML = ""; // Clears only the fish, not the status elements
        for (let i = 0; i < myoptions.NumFishGroups; i++) {
          let species = pick(myoptions.FishSpecies.split(" "));
          let numFish = 1;
          if (!myoptions.SingleFishOnly && Math.random() * 100 > 100 - myoptions.PercentSchools) {
          ...
          let school = generateSchool(numFish, species, hueShift);
          fishContainer.appendChild(school); // Append schools to the fish container
          loop(school);
        }
    }
    ...
    function simulateStatus() {
        setInterval(() => {
          ...
          const randomTemp = (20 + Math.random() * 10).toFixed(1); // 20-30°C
          const randomPH = (6 + Math.random() * 2).toFixed(1); // pH 6.0-8.0
          const randomLength = (50 + Math.random() * 150).toFixed(1); // 50-200cm
      
          updateAdditionalInfo(randomTemp, randomPH, randomLength);
