...
// Initialize aquarium with 60 fishes
...
function updateTankStatus(status) {
  ...
  if (status === "suck") {
    fishes.forEach((fish) => {
      fish.classList.add("dead"); // Mark fish as dead
      fish.style.transition = "opacity 2s, transform 2s";
      fish.style.opacity = 0; // Fade out
      fish.style.transform = "translateY(50px) rotate(90deg)"; // Drop and rotate
    });
  } else {
    fishes.forEach((fish) => {
      fish.classList.remove("dead"); // Restore fish
      fish.style.transition = "opacity 2s";
      fish.style.opacity = 1; // Fade back in
      moveFishSmoothly(fish); // Resume movement
    });
  }
}
